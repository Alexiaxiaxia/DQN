...
class ConvNet(nn.Module):
    def __init__(self):
        ...
        # self.fc_0 = nn.Linear(1588, 8 * 8 * 8 + N_TILE * N_STATE)
        # 32 + N_TILE * N_STATE
        # conv_output_size + N_TILE * N_STATE
        self.fc_0 = nn.Linear(32 * 7* 7 + N_TILE * STATE_LEN , 8 * 8 * 8 + N_TILE * N_STATE)
        self.fc_1 = nn.Linear(8 * 8 * 8 + N_TILE * N_STATE, 8 * 8 * 8 + N_TILE * N_STATE)
        # initialization    
...
class DQN(object):
    ...
    def choose_action(self, s, epsilon, idling):
        ...
        x = torch.FloatTensor(s[0][0])
        state = torch.FloatTensor(s[0][1])
        # print(x.shape)
        if USE_GPU:
            x = x.cuda()
            state = state.cuda()
        ...
        if np.random.uniform() >= epsilon:
            # greedy case
            x = x[-STATE_LEN:].unsqueeze(0)
            state = state[-STATE_LEN:].unsqueeze(0)
    ...
    def learn(self):
        ...
        # obses_i, obses_v, actions, rewards, obses_p_i, obses_p_v, dones = self.replay_buffer.sample(BATCH_SIZE)
        
        # # transfer to tensor
        # obses_i = torch.FloatTensor(obses_i)
        # obses_v = torch.FloatTensor(obses_v)
        # actions = torch.LongTensor(actions)
        # rewards = torch.FloatTensor(rewards)
        # obses_p_i = torch.FloatTensor(obses_p_i)
        # obses_p_v = torch.FloatTensor(obses_p_v)
        # dones = torch.FloatTensor(dones)
        
        # if USE_GPU:
        #     obses_i, obses_v, actions, rewards, obses_p_i, obses_p_v, dones = \
        #         obses_i.cuda(), obses_v.cuda(), actions.cuda(), rewards.cuda(), \
        #         obses_p_i.cuda(), obses_p_v.cuda(), dones.cuda()
        
        # q_eval = self.pred_net(obses_i, obses_v)
        # q_eval = torch.gather(q_eval, 1, actions.unsqueeze(1)).squeeze(1)
        
        # q_next = self.target_net(obses_p_i, obses_p_v)
        # q_next = torch.max(q_next, -1)[0]
        # q_target = rewards + GAMMA * q_next * (1 - dones)
        
        # loss = self.loss_function(q_eval, q_target.detach())
       ...
       # target parameter update
        ...
        obses_i  = obses_i[:, -STATE_LEN:, :, :]
        obses_v  = obses_v[:, -STATE_LEN:]
        actions = actions
        rewards = rewards
        obses_p_i = obses_p_i[:, -STATE_LEN:, :, :]
        obses_p_v = obses_p_v[:, -STATE_LEN:]
        ...
        # with torch.no_grad():
        #     q_next = self.target_net(obses_p_i, obses_p_v)
        #     q_next_pred = self.pred_net(obses_p_i, obses_p_v)
        #     max_actions = torch.argmax(q_next_pred, dim=-1, keepdim=True)
        #     q_next = torch.gather(q_next, 1, max_actions).squeeze(1)
        #     q_target = rewards + GAMMA * q_next * (1 - dones)
